...
const LoginPage = () => {
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      ...
      if (activeTab === 'signin') {
        // Mock sign-in logic
        if (formData.email.toLowerCase().includes('ngo') || formData.email.toLowerCase().includes('alpha')) {
          userData = {
            firstName: 'Animal Angels',
            role: 'NGO/Organization',
            ngo_id: 'ngo_12345'
          };
        } else if (formData.email.toLowerCase().includes('karjat')) {
          userData = {
            firstName: 'Karjat Helpers',
            role: 'NGO/Organization',
            ngo_id: 'ngo_67890'
          };
        } else {
        ...
        const isNgo = roleTitle === 'NGO/Organization';

        userData = {
          firstName: isNgo ? formData.organizationName : formData.firstName,
          role: roleTitle,
          ngo_id: isNgo ? `ngo_${Date.now()}` : null
        };
      ...
      navigate('/'); // Always navigate to the homepage after login/signup
      // --- پہلے تھا: navigate((userData.role === 'NGO/Organization') ? '/dashboard' : '/'); ---
