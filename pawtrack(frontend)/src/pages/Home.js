        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div className="animate-on-scroll hero-text-content space-y-8">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              {/* Show "Report" button only if NOT an NGO */}
              {(!user || user.role !== 'NGO/Organization') && (
              ...
              {/* Scan QR button (class name unchanged) */}
...
// --- Problem Component (No changes needed) ---
const Problem = () => (
...
// --- AnimatedCounter Component (No changes needed) ---
const AnimatedCounter = ({ end, duration = 2000, suffix = "" }) => {
    ...
    const observer = new IntersectionObserver(([entry]) => {
      if (entry.isIntersecting) {
        ...
        const animate = (currentTime) => {
          ...
          // Ease-out quartic animation function
  ...
  }, [end, duration]); // Re-run effect if end or duration changes

  return <span ref={ref}>{count.toLocaleString()}{suffix}</span>;
};

// --- ImpactStats Component (No changes needed) ---
const ImpactStats = () => {
  const stats = [
  ...
  return (
    <section className="stats-section"> {/* Assumes this has the dark background */}
      <div className="stats-pattern"></div>
      <div className="container relative">
        <div className="text-center mb-16 animate-on-scroll">
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-white mb-6 text-balance">Making a Real Impact</h2>
        </div>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
          {stats.map((stat, index) => (
            <div key={stat.label} className="animate-on-scroll group" style={{ animationDelay: `${index * 0.1}s` }}>
              {/* --- ADD INLINE STYLE HERE for transparent background --- */}
              <div
                className="stat-card"
                style={{
                  backgroundColor: 'transparent',
                  border: 'none', // Remove border if it exists via CSS
                  boxShadow: 'none', // Remove shadow if it exists via CSS
                  padding: '1rem 0' // Adjust padding if needed
                }}
              >
                {/* --- CHANGE INLINE STYLE HERE for visibility --- */}
                <div
                  className="stat-number"
                  style={{ color: '#FFFFFF' }} // White color for numbers
                >
                ...
                <div
                  ...
                  style={{ color: 'rgba(255, 255, 255, 0.8)' }} // Light grey/white for labels
                >
                  {stat.label}
                </div>
              </div>
...
// --- JoinMission Component (Updated Logic) ---
const JoinMission = ({ user }) => (
  <section className="join-mission animate-on-scroll">
    <div className="container">
      ...
      <div className="join-mission-buttons">
        {/* Show "Report" if logged out OR not an NGO */}
        {(!user || user.role !== 'NGO/Organization') && (
          <Link to="/report" className="btn btn-primary btn-lg group btn-glow">
            <Camera className="w-5 h-5 mr-2 group-hover:rotate-12 transition-transform" />
            Report an Animal
...
const Home = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      <Header />
      <main>
        {/* Pass user prop down to Hero and JoinMission */}
        <Hero user={user} />
        <Problem />
        <HowItWorks />
        <ImpactStats />
        <JoinMission user={user} />
      </main>
      <Footer />
    </div>
  );
